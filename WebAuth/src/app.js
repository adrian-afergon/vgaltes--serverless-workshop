...
Amplify.configure({
    Auth: {
        ...
        userPoolId: 'eu-west-1_zCHuiDkX1',

        // OPTIONAL - Amazon Cognito Web Client ID (26-char alphanumeric string)
        userPoolWebClientId: '6b4qkg2a856eft37hl2nh30ljt',
        ...
        endpoints: [
            {
                ...
                endpoint: "https://lgb7559glc.execute-api.eu-west-1.amazonaws.com"
            }
        ]
    }
});
...
RequestToSecuredEndpointButton.addEventListener('click', async evt => {
  ...
  await API.get("MyAPIGatewayAPI", '/devafergon/api/getTogethers', myInit).then(response => {
...
RequestToNonSecuredEndpointButton.addEventListener('click', async evt => {
  await API.get("MyAPIGatewayAPI", '/devafergon/api/testAutoSubscribe').then(response => {
    console.log(response);
  }).catch(error => {
      console.log(JSON.stringify(error))
  });
